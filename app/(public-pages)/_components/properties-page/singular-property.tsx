...
const SingularProperty = ({ id }: { id: string }) => {
    const fadeInUp = {
    ...
    const property = segregatedproperties.find((property) => property.id === id);
    if (!property) {
        return (
            <motion.div
                {...fadeInUp}
    ...
    const { title, description, location, features, status, price, mainImage, category, benefit, payment } = property;
    const hasMultipleImages = mainImage?.image && mainImage.image.length > 1;
            ...
            >
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                        ...
                        <span className={`px-3 py-1 text-xs sm:text-sm font-medium rounded-full ${status === Status.Available
                ...
                >
                    {price.map((price) => (
                        <div key={price.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-4">
                                {payment.map((payment, index) => (
                                    <div key={`${index}-payment`} className="bg-white p-4 rounded-lg shadow-sm">
                                        <h3 className="font-semibold text-gray-800">{payment.id}</h3>
                                        <p className="text-gray-600 text-sm mt-1">
                                            {payment.id.includes('Initial') ?
                                                '50% upfront payment' :
