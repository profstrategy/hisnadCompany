...
export async function POST(req: NextRequest) {
  try {
    ...
    const validStaus = ["paid", "active"];
    // Check if user exists
    if (
      !getUserByThierEmail ||
      !validStaus.includes(getUserByThierEmail.status)
    ) {
      return NextResponse.json(
        {
          message: "User not found or incomplete registration",
        },
        { status: 404 }
      );
    }
    ...
    const reset_password_token = generateResetPasswordToken(
      getUserByThierEmail.id
    );
    ...
    return NextResponse.json(
      {
        message: `Reset password initiated successfully, Link sent to ${updatedUser.email}`,
